...
\begin{compactitem}
  ...
  \end{compactitem}
  \item The ``bypass methods'' part of the definition allows operations (e.g. |*|, |%| and |norm|) to execute directly on values \emph{encoded} in the representation type (|Long| in this case), instead of \emph{decoding} them back to |(Int, Int)| before executing the operation. We explain how bypass methods are defined and used later or, in \S\ref{sec:ildl:method}.
\end{compactitem}
