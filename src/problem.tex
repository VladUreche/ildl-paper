...
Unfortunately, as our algorithm recursively computes the result, it creates linearly many pairs of integers, allocating them in the heap memory. If we run this algorithm without any compiler
and virtual machine %% \vlad{yes, otherwise aggressive inlining (due to the monomorphic code) and escape analysis reduce the gap to < 50%}
optimizations, each GCD operation takes around 130
microseconds (on the same setup as used for our full experiments in \S\ref{sec:benchmarks}):
...
trait, object and package definitions:
...
How does the compiler know what encoding/decoding procedures to use to convert the pairs of integers to the long integer representation (\S\ref{sec:ildl:ldl})? Even worse, what if the code is compiled separately, in a different compiler run (\S\ref{sec:ildl:separate-compilation})?
