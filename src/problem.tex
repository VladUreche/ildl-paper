...
\begin{itemize}
  \item[(1)] Provides a direct representation, which does not require any pointer de-referencing;
...
\noindent
Transformation description objects are described in more detail in Section \ref{sec:ildl}, but we can already hint at their components:
\begin{itemize}
...
\subsubsection{The transformations scope} is delimited by the |adrt| marker method, which behaves much like a keyword. Values, methods and classes defined inside the |adrt| scope are visible outside, since the scope is inlined early in the compilation pipeline:
