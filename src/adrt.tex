...
It may seem natural that the value |t| will be transformed to use the |V| representation type: first, converting from |T| to |U| and then from |U| to |V|. Unfortunately, the current theory does not allow a value to undergo multiple levels of ADR transformations, and this extension is not trivial. For example, the |@repr| annotation would have to take a list of transformation description objects instead of one. Therefore, in the current approach, we disallow cascading scopes:
...
Thanks to the \bridge{} phase, class |Z| conforms to the trait (interface) |X|, any call going through the interface will execute as expected, albeit, in this case, less efficiently.
