...
but forces the recompilation of all the code that references the old
object layout.
...
important overheads, both for profiling and for the recompilation.
Thus, runtime system decisions to change the low-level object layout
are both global and expensive.
...
marked as specialized or as a value class (and assuming it satisfies
...
making it impossible to represent our ``complex numbers'' as a long
integer value even if we had complete control over the Scala library.
...
well-defined scopes that can include anything from an expression to
...
In this way, the programmer is responsible for correctly stating a)
what is the data representation transformation; and b) the program
scope in which it is applicable. The compiler (i.e., our system) is
responsible for a) automatically deciding when to apply the
transformation and when to revert it, thus ensuring correct
interchange between representations; b) enriching the transformation
with automatically computed bridge code that ensures correctness
relative to overriding, dynamic dispatch, and more.  Thus, our
approach adheres to the principle of separating the reusable,
general and provably correct mechanism from the programmer-driven
policy, which may contain incorrect decisions
